...
export default async function handler(req,res){
 ...
 let processed=0,deliveries=0;for(const e of events||[]){const {data:hooks}=await db.from('api_webhooks').select('id,url,status,failure_count,secret_ciphertext').eq('status','active').contains('events',[e.event_type]);let allAttempted=true;for(const h of hooks||[]){try{const r=await deliver(h,e);if(!r.duplicate)deliveries++;}catch(x){allAttempted=false;console.error('[merveil-webhook-process]',h.id,e.event_id,x);}}
