  ...
  try {
    ...
    if (resource === "conversations") {
      ...
      if (method === "POST" && action === "presence") {
        ...
        await sb.from("presence").upsert(
          { user_id: user.id, status, updated_at: nowIso },
          { onConflict: "user_id" }
        );
        // Keep profiles.last_seen_at in step with heartbeats so "recently
        // active" ranking (My Circle offline tier) has real data even
        // after the live presence row goes stale/offline.
        sb.from("profiles").update({ last_seen_at: nowIso }).eq("id", user.id).then(() => {}).catch(() => {});
        return sendJson(res, 200, { ok: true });
      }
      ...
      if (method === "GET" && action === "directory") {
        ...
        let query = sb.from("profiles")
          .select("id,name,avatar_url,role_label,passport_tier,discoverable,last_seen_at")
          .neq("id", user.id)
          .limit(300);
    ...
    if (resource === "connections" && action === "list" && method === "GET") {
      ...
      const { data: profiles } = otherIds.length
        ? await anonClient().from("profiles").select("id, name, avatar_url, profession, company_name, account_type, passport_tier, role_label, last_seen_at").in("id", otherIds)
        : { data: [] };
