...
function adminClient() {
  ...
  // ROOT CAUSE (Aug 2026): this Supabase project was created through the
  // Vercel Marketplace integration, not the Supabase dashboard directly.
  // Vercel Marketplace auto-syncs the secret key under the env var name
  // `SUPABASE_SECRET_KEY` (see Supabase's "Vercel Marketplace" docs) —
  // NOT `SUPABASE_SERVICE_ROLE_KEY`. This function was only ever checking
  // the latter, so it was reading a manually-pasted, stale/mismatched key
  // instead of the one Vercel keeps in sync automatically. Check the
  // Marketplace-synced name first.
  const key =
  ...
  if (!key) {
    ...
    throw new Error(
      "Server misconfiguration: missing SUPABASE_SECRET_KEY (or legacy SUPABASE_SERVICE_ROLE_KEY) in Vercel environment variables."
    );
  }
...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "conversations") {
      ...
      if (method === "POST" && action === "presence") {
        ...
        await sb.from("presence").upsert(
          { user_id: user.id, status: body.status || "online", updated_at: new Date().toISOString() },
          { onConflict: "user_id" }
        );
        return sendJson(res, 200, { ok: true });
      }
    ...
    if (resource === "diag" && req.query.action === "svc-check") {
      // Report on every candidate var name so a future regression is
      // visible at a glance instead of requiring re-diagnosis from scratch.
      const marketplaceKey = process.env.SUPABASE_SECRET_KEY || "";
      const legacyKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_ROLE || "";
      const key = marketplaceKey || legacyKey;
      const out = {
        usingVar: marketplaceKey ? "SUPABASE_SECRET_KEY (Marketplace-synced)" : (legacyKey ? "SUPABASE_SERVICE_ROLE_KEY (legacy/manual)" : "none found"),
        hasSUPABASE_SECRET_KEY: !!marketplaceKey,
        hasSUPABASE_SERVICE_ROLE_KEY: !!legacyKey,
    ...
    if (resource === "webrtc" && action === "ice-servers" && method === "GET") {
      ...
      if (!keyId || !apiToken) {
        return sendJson(res, 500, { error: "Calling isn't configured on the server yet (missing CLOUDFLARE_TURN_KEY_ID / CLOUDFLARE_TURN_API_TOKEN)." });
      }
      try {
        ...
        if (!cfRes.ok) {
          ...
          return sendJson(res, 502, { error: `TURN credential request failed: ${errText}` });
        }
        const data = await cfRes.json();
        return sendJson(res, 200, { iceServers: data.iceServers });
      } catch (err) {
        return sendJson(res, 500, { error: `TURN credential request failed: ${err.message}` });
      }
    }
    ...
    if (resource === "calls" && action === "create" && method === "POST") {
      ...
      if (!conn) return sendJson(res, 403, { error: "You can only call someone you're connected with." });
