...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "conversations") {
      ...
      // Live presence: online if heartbeat within 90s. Online users first.
      ...
      if (method === "GET") {
        ...
        if (error) return sendJson(res, 400, { error: error.message });
        // BUG FIX: sort by last message time (not created_at) so new activity
        // bubbles to the top like WhatsApp/Telegram. Also surface last
        // message timestamp + unread count for the list UI.
