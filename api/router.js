...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "world") {
      ...
      if (method === "GET") {
        // Never cache an empty feed (that left World blank after real posts
        // existed). Short CDN TTL only when content is present.
    ...
    if (resource === "comments") {
      ...
      if (method === "POST") {
        ...
        const ins = await writer
          .from("comments")
          .insert({ target_type: body.targetType, target_id: String(body.targetId), user_id: commenterId, body: text })
          .select()
          .maybeSingle();
        data = ins.data;
        error = ins.error;
