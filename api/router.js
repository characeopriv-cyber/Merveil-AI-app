...
export default async function handler(req, res) {
  try {
        ...
        if (ids.length) {
          ...
          for (const m of recentMsgs || []) {
            ...
            const isUnread =
              ...
              !readBy.includes(String(user.id)) &&
              !m.read_at;
    ...
    if (resource === "connections") {
      ...
      if (action === "status" && method === "GET") {
        ...
        const { data: row } = await svc
          ...
          .or(
            `and(user_id.eq.${user.id},connected_user_id.eq.${otherId}),and(user_id.eq.${otherId},connected_user_id.eq.${user.id})`
          )
          .order("created_at", { ascending: false })
          .limit(1)
          .maybeSingle();
        if (!row) return sendJson(res, 200, { status: "none", connectionId: null });
        const direction = row.user_id === user.id ? "outgoing" : "incoming";
        return sendJson(res, 200, {
          status: row.status,
          connectionId: row.id,
          direction,
      ...
      // GET /api/connections?action=statuses&userIds=id1,id2,id3 — bulk
      // version of the single-status lookup above. World's feed/reels
      // Connect buttons render many creators at once; without this they
      // had no way to know a request was already pending/accepted and
      // always rendered as a fresh "Connect" no matter what — this is
      // what makes those buttons reflect real state.
      if (action === "statuses" && method === "GET") {
        const idsParam = req.query.userIds || "";
        const ids = [...new Set(String(idsParam).split(",").map((s) => s.trim()).filter(Boolean).filter((id) => id !== user.id))].slice(0, 100);
        if (!ids.length) return sendJson(res, 200, { statuses: {} });
        const { data: rows } = await svc
          .from("connections")
          .select("id, user_id, connected_user_id, status, created_at")
          .or(`and(user_id.eq.${user.id},connected_user_id.in.(${ids.join(",")})),and(user_id.in.(${ids.join(",")}),connected_user_id.eq.${user.id})`)
          .order("created_at", { ascending: false });
        const statuses = {};
        for (const row of rows || []) {
          const otherId = row.user_id === user.id ? row.connected_user_id : row.user_id;
          if (statuses[otherId]) continue; // keep most recent only (rows already ordered desc)
          statuses[otherId] = {
            status: row.status,
            connectionId: row.id,
      ...
      if (action === "suggestions" && method === "GET") {
        const { data: me } = await anonClient()
          .from("profiles")
          .select("profession, skills, languages, city, country, account_type")
          .eq("id", user.id)
          .maybeSingle();
        const mySignals = [me?.profession, ...(me?.skills || []), ...(me?.languages || []), me?.city, me?.country]
          .filter(Boolean)
        ...
        for (const r of existingRows || []) {
          ...
          relatedIds.add(r.user_id === user.id ? r.connected_user_id : r.user_id);
        }
        ...
        const scored = (others || [])
          ...
          .map((p) => {
            ...
            let score = 0;
            let reason = null;
            if (me?.profession && p.profession && String(p.profession).toLowerCase() === String(me.profession).toLowerCase()) {
              score += 3;
              reason = `Also works in ${p.profession}`;
            }
            if (me?.city && p.city && p.city === me.city) {
              score += 2;
              reason = reason || `Also based in ${p.city}`;
            }
            if (me?.country && p.country && p.country === me.country && !reason) {
              score += 1;
      ...
      if (action === "request" && method === "POST") {
        ...
        const okRate = await checkRateLimit(anonClient(), `conn_${user.id}`, 30);
      ...
      if (action === "remove" && method === "POST") {
        ...
        if (row.user_id !== user.id && row.connected_user_id !== user.id) {
      ...
      if (action === "block" && method === "POST") {
        ...
        if (!targetId) return sendJson(res, 400, { error: "userId required" });
        if (String(targetId) === String(user.id)) return sendJson(res, 400, { error: "You can't block yourself." });
        ...
        await svc
          ...
          .delete()
          .or(
            `and(user_id.eq.${user.id},connected_user_id.eq.${targetId}),and(user_id.eq.${targetId},connected_user_id.eq.${user.id})`
          );
        return sendJson(res, 200, { ok: true });
      }
      ...
      if (action === "list" && method === "GET") {
        ...
        let query = svc
          ...
          .select("id, user_id, connected_user_id, status, created_at, responded_at");
        if (kind === "incoming") {
          query = query.eq("connected_user_id", user.id).eq("status", "pending");
        ...
        const otherIds = [
          ...new Set(
            (rows || []).map((r) => (r.user_id === user.id ? r.connected_user_id : r.user_id))
          ),
        ];
        ...
        const items = (rows || []).map((r) => {
          ...
          return {
            ...
            direction: r.user_id === user.id ? "outgoing" : "incoming",
    ...
    if (resource === "calls" && action === "create" && method === "POST") {
      ...
      // not permit INSERT from the caller's session — which produced the
      // "new row violates row-level security policy for table calls" alert.
      // Authorization (connection / shared convo / blocks / restrictions /
      // callee prefs) is enforced above; the service client only writes the
    ...
    if (resource === "comments") {
      // GET /api/comments?action=counts&targetType=world_post&targetIds=id1,id2
      // Bulk comment counts for a page of items (e.g. every reel currently
      // loaded in World). Without this, World Reels had no way to show a
      // comment count at all — only Like/Super did — which read as the
      // count "not working" even though comments themselves worked fine.
      if (method === "GET" && action === "counts") {
        const targetType = req.query.targetType;
        const ids = [...new Set(String(req.query.targetIds || "").split(",").map((s) => s.trim()).filter(Boolean))].slice(0, 200);
        if (!targetType || !ids.length) return sendJson(res, 200, { counts: {} });
        const { data, error } = await anonClient()
          .from("comments")
          .select("target_id")
