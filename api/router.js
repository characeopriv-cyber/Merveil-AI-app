...
<p>Opening <a href="${esc(dest)}" style="color:#06B6D4">Merveil</a>…</p>
    ...
    if (resource === "conversations") {
      ...
      if (convId && segments[2] === "messages") {
        ...
        if (method === "POST") {
          ...
          try {
            const others = (convo.participant_ids || []).filter((id) => String(id) !== String(user.id));
            ...
            try {
              const { data: me } = await adminClient().from("profiles").select("name").eq("id", user.id).maybeSingle();
      ...
      if (method === "GET") {
        ...
        const listId = user?.id || citizen?.id;
        if (!listId) return sendJson(res, 200, { conversations: [] });
        let listClient = sb;
        if (!token) {
          try { listClient = adminClient(); } catch { return sendJson(res, 200, { conversations: [] }); }
        }
    ...
    if (resource === "company") {
      ...
      if (method === "POST" && cAction === "create") {
        ...
        // Align Passport to company capability when creating org
        try {
          await svc.from("profiles").update({ passport_tier: "company", company_name: name, account_type: "company" }).eq("id", actorId);
        } catch {}
