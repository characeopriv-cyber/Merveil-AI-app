...
<p>Opening <a href="${esc(dest)}" style="color:#06B6D4">Merveil</a>…</p>
    ...
    // ------------------------------------------------------- /api/webhooks/stripe
    // Stripe → credit wallet on payment_intent.succeeded (idempotent)
    if (resource === "webhooks" && (req.query.provider === "stripe" || action === "stripe")) {
      ...
      try {
        const body = await readBody(req);
        // Without stripe SDK signature verify: accept JSON if no secret, or trust Vercel internal only when secret set check header presence
        if (whSecret) {
          const sig = req.headers["stripe-signature"];
          if (!sig) return sendJson(res, 400, { error: "Missing stripe-signature" });
          // Lightweight: still parse body; production should use stripe.webhooks.constructEvent with raw body
          event = body;
          if (body?.type) event = body;
        } else {
          event = body;
        }
      } catch (e) {
      ...
      const piId = obj.id;
      const metaUser = obj.metadata?.user_id;
      const amountFils = Number(obj.amount_received || obj.amount || 0);
      const amount = amountFils / 100;
      if (!piId || !metaUser || !(amount > 0)) {
        return sendJson(res, 200, { received: true, skipped: "missing fields" });
      }
      const svc = adminClient();
      const { data: existing } = await svc.from("payment_intents").select("*").eq("provider_ref", piId).maybeSingle();
      if (existing?.status === "succeeded" || existing?.status === "credited") {
        return sendJson(res, 200, { received: true, duplicate: true });
      }
      if (existing) {
        await svc.from("payment_intents").update({
          status: "succeeded",
          meta: { ...(existing.meta || {}), webhook: true },
          updated_at: new Date().toISOString(),
        }).eq("id", existing.id);
      } else {
        await svc.from("payment_intents").insert({
          ...
          amount,
          currency: "AED",
          purpose: obj.metadata?.purpose || "wallet_topup",
          status: "succeeded",
          provider: "stripe",
          provider_ref: piId,
        });
      }
      // Credit ledger if not already
      const { data: prior } = await svc.from("wallet_ledger")
        .select("id").eq("reference_type", "payment_intent").eq("reference_id", existing?.id || piId).eq("kind", "topup").limit(1);
      if (prior?.length) {
      ...
      if (!w) {
        const ins = await svc.from("user_wallets").upsert({ user_id: metaUser, available: 0, pending: 0, currency: "AED" }).select().maybeSingle();
        w = ins.data || { available: 0, pending: 0 };
      }
      const available = Number(w.available || 0) + amount;
      await svc.from("user_wallets").update({ available, updated_at: new Date().toISOString() }).eq("user_id", metaUser);
      await svc.from("wallet_ledger").insert({
        user_id: metaUser,
        direction: "credit",
        amount,
        ...
        description: `Stripe top-up AED ${amount.toFixed(2)}`,
        reference_type: "payment_intent",
        reference_id: existing?.id || piId,
      });
      try {
      ...
      if (existing) {
        await svc.from("payment_intents").update({ status: "credited" }).eq("id", existing.id);
      }
      return sendJson(res, 200, { received: true, credited: amount, userId: metaUser });
    }
    ...
    if (resource === "wallet") {
      ...
      if (method === "POST" && wAction === "topup") {
        ...
        if (!stripeKey) {
          // Sandbox: credit immediately (dev / pre-processor)
          ...
          try {
            const result = await postLedger({
              ...
              amount,
              kind: "topup",
              description: `Sandbox top-up AED ${amount.toFixed(2)}`,
            ...
            return sendJson(res, 200, {
              ...
              message: "Sandbox credit applied. Connect STRIPE_SECRET_KEY for real card payments.",
            });
          } catch (e) {
            return sendJson(res, 400, { error: e.message });
          }
        ...
        // Stripe PaymentIntent (real card path)
        try {
          const amountFils = Math.round(amount * 100); // AED minor units
          const piRes = await fetch("https://api.stripe.com/v1/payment_intents", {
            ...
            body: new URLSearchParams({
              ...
              description: `Merveil wallet top-up AED ${amount.toFixed(2)}`,
            }).toString(),
          });
          ...
          const { data: intentRow } = await svc.from("payment_intents").insert({
            user_id: actorId,
            amount,
            currency: "AED",
            purpose,
            status: pi.status || "requires_payment",
            provider: "stripe",
            provider_ref: pi.id,
          ...
          return sendJson(res, 200, {
            ...
            message: "Confirm payment with Stripe.js using clientSecret, then wallet credits on webhook.",
          });
        } catch (e) {
          return sendJson(res, 400, { error: e.message });
        }
