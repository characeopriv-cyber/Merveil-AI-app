...
<p>Opening <a href="${esc(dest)}" style="color:#06B6D4">Merveil</a>…</p>
    ...
    if (resource === "wallet") {
      ...
      if (method === "POST" && wAction === "topup") {
        ...
        // Stripe path (minimal — expand when keys live)
        try {
          ...
          // Placeholder: real Stripe PaymentIntent create goes here
          // const stripe = require("stripe")(stripeKey);
          // const pi = await stripe.paymentIntents.create({ amount: Math.round(amount * 100), currency: "aed", ... });
          return sendJson(res, 200, {
            mode: "stripe",
            intent: intentRow,
            message: "Stripe key detected — implement PaymentIntent create in /api/wallet?action=topup.",
            clientSecret: null,
          });
        } catch (e) {
          return sendJson(res, 400, { error: e.message });
