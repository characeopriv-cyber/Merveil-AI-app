...
export default async function handler(req, res) {
  try {
    ...
    if (resource === "world") {
      ...
      if (method === "GET") {
        // Edge-friendly caching: public feed is cacheable for a short TTL
        // at the CDN (Vercel/Cloudflare) so 1k concurrent reel viewers share
        // one origin response instead of hammering PostgREST.
    ...
    if (resource === "people") {
      ...
      if (action === "profile" && method === "GET") {
        ...
        const { data, error } = await anonClient()
          .from("profiles")
          .select("id, name, avatar_url, cover_video_url, junction_id, passport_tier, country, bio, created_at, account_type, company_name, city, profession, languages")
          .eq("id", userId)
          .maybeSingle();
        if (error) return sendJson(res, 400, { error: error.message });
