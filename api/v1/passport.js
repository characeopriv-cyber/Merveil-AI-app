...
export default async function handler(req, res) {
  ...
  if (!auth.app.scopes.includes('passport:read')) return json(res, 403, { error: 'insufficient_scope' });
  ...
  if (!data) return json(res, 404, { error: 'passport_not_found' });

  await supabaseAdmin.from('api_applications').update({ last_used_at: new Date().toISOString() }).eq('id', auth.app.id);
  await supabaseAdmin.from('api_usage_events').insert({ application_id: auth.app.id, endpoint: '/api/v1/passport', method: 'GET', status_code: 200, latency_ms: Date.now() - started });
