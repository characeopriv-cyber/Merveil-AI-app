...
export default async function handler(req, res) {
  ...
  if (!check.ready) return res.status(422).json({ ok: false, status: 'blocked', code: 'BUILD_CHECK_BLOCKED', check });
  ...
  try {
    ...
    await db.from('developer_builds').update({ status, logs: String(result.logs || '').slice(-30000), finished_at: finished }).eq('id', row.id).eq('owner_user_id', uid);
    return res.status(200).json({ ok: status === 'success', status, project, build: { ...result, startedAt: started, finishedAt: finished, buildId: row.id, buildCheck: check } });
  } catch (error) {
    const message = error?.message || String(error);
    await db.from('developer_builds').update({ status: 'failed', logs: message.slice(0, 30000), finished_at: new Date().toISOString() }).eq('id', row.id).eq('owner_user_id', uid);
    return res.status(500).json({ ok: false, status: 'failed', error: message, buildId: row.id });
  }
}
