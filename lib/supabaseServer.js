...
export function getAccessToken(req) {
  const fromCookie = parseCookies(req)[COOKIE_NAME] || null;
  if (fromCookie) return fromCookie;
  // Fallback: Authorization Bearer (helps mobile PWAs / cookie-blocked contexts)
  try {
...
/** Drop in-memory session so logout cannot be undone by a warm instance cache. */
export function forgetSession(userId) {
  if (!userId) {
    recentSessionsByUser.clear();
    return;
  }
 ...
 * Returns { token, user, jwtSub }. Never clears cookies on a refresh race.
 * jwtSub is ALWAYS populated when any cookie still names a user so calls,
 * directory, and presence never 401 a still-signed-in citizen mid-refresh.
 */
export async function getSession(req, res) {
  const token = getAccessToken(req);
  const refreshToken = getRefreshToken(req);
  // Always extract sub from whichever cookie still carries a JWT payload.
  // Expired access tokens still decode — that is enough for service-role
  // directory / rewards / profile-views without forcing sign-out/sign-in.
  let jwtSub =
    (token ? decodeJwtSub(token) : null) ||
    (refreshToken ? decodeJwtSub(refreshToken) : null) ||
    null;
  ...
  if (token) {
    try {
      ...
      if (!error && data?.user) {
        ...
        return { token, user: data.user, jwtSub: data.user.id || jwtSub };
      }
    } catch {
  ...
  if (jwtSub) {
    ...
    if (recent?.accessToken) {
      ...
      return {
        token: recent.accessToken,
        user: recent.user,
        jwtSub: recent.user?.id || jwtSub,
      };
    }
  }
  ...
  if (refreshToken) {
    try {
      ...
      if (!refreshErr && refreshed?.session) {
        ...
        return { token: access, user: u, jwtSub: u?.id || jwtSub || decodeJwtSub(access) };
      }
      // Refresh failed (often "already used") — check race cache again
      const sub =
        jwtSub ||
        (token ? decodeJwtSub(token) : null) ||
        decodeJwtSub(refreshToken);
      if (sub) {
        ...
        if (recent?.accessToken) {
          ...
          return {
            token: recent.accessToken,
            user: recent.user,
            jwtSub: recent.user?.id || sub,
          };
        }
        // Soft identity: keep jwtSub so service-role paths still authorize
        jwtSub = sub;
      }
    } catch {
      /* fall through */
  ...
  // 4) Last resort: expired access/refresh JWT still names a user. Do NOT invent a
  ...
  const finalSub =
    ...
    (refreshToken ? decodeJwtSub(refreshToken) : null) ||
    null;
  // Also scan recent session map for any still-warm user (covers cookie lag)
  if (!finalSub && recentSessionsByUser.size) {
    let newest = null;
    for (const s of recentSessionsByUser.values()) {
      if (!newest || s.at > newest.at) newest = s;
    }
    if (newest && Date.now() - newest.at < 5 * 60 * 1000 && newest.user?.id) {
      if (res && newest.refreshToken && newest.accessToken) {
        setSessionCookie(res, newest.accessToken, newest.refreshToken);
      }
      return {
        token: newest.accessToken || null,
        user: newest.user,
        jwtSub: newest.user.id,
      };
    }
  }
...
/** Allowed browser origins for CORS (prod + local + preview). */
const CORS_ORIGINS = new Set([
  "https://junction.technology",
  "https://www.junction.technology",
  "https://merveil.ai",
  "https://www.merveil.ai",
  "http://localhost:5173",
  "http://localhost:3000",
  "http://127.0.0.1:5173",
  "http://127.0.0.1:3000",
...
export function applyCors(req, res) {
  try {
    ...
    if (origin && (CORS_ORIGINS.has(origin) || /\.vercel\.app$/i.test(origin) || /\.junction\.technology$/i.test(origin))) {
      res.setHeader("Access-Control-Allow-Origin", origin);
      res.setHeader("Vary", "Origin");
    } else if (!origin) {
      // Same-origin / server-to-server
      res.setHeader("Access-Control-Allow-Origin", "https://www.junction.technology");
    }
    res.setHeader("Access-Control-Allow-Credentials", "true");
    res.setHeader("Access-Control-Allow-Methods", "GET,POST,PUT,PATCH,DELETE,OPTIONS");
    res.setHeader(
      "Access-Control-Allow-Headers",
      "Content-Type, Authorization, X-Requested-With, X-Merveil-Client, Accept, Origin"
    );
    res.setHeader("Access-Control-Expose-Headers", "X-Merveil-Request-Id, Content-Disposition");
