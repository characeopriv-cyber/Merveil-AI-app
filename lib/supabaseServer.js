...
export function getAccessToken(req) {
  // After getSession() has conclusively checked the request, never expose a
  // dead access cookie to downstream fallback auth logic. This prevents a
  // stale JWT subject from being mistaken for a live session.
  if (req?.__merveilAuthChecked && !req.__merveilAuthenticated) return null;

  // Prefer an explicit Authorization bearer token over the legacy/custom
  // cookie. The browser Supabase session can be newer than jx_at after a
  // refresh race, and the old ordering made the stale cookie win forever.
  try {
...
export async function getSession(req, res) {
  ...
  if (token) {
    try {
      ...
      if (!error && data?.user) {
        // If the request arrived with a fresh bearer token, synchronize the
        // access cookie as well. This heals the custom session bridge without
        // requiring a second login. Never invent a refresh token here.
        if (res && tokenFromAuthorization) {
          setSessionCookie(res, token, refreshToken || null);
        }
  ...
  // 2) Another concurrent request already rotated for this user
  if (jwtSub) {
