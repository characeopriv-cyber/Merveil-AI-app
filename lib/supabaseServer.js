...
// Public Supabase project identifiers only. Secrets stay server-side in Vercel env.
...
function cookieString(name, value, maxAgeSeconds) {
  ...
  if (isProd) parts.push("Secure", "Domain=.junction.technology");
  return parts.join("; ");
}
...
// JWT decoding is retained only for correlation/debugging. It is NEVER an
// authentication decision and is never returned as an authenticated user id.
...
// Warm-instance session coordination prevents refresh-token races without
// treating an unverified JWT payload as an authenticated identity.
const refreshInFlight = new Map();
const recentSessionsByUser = new Map();

function rememberSession(userId, accessToken, refreshToken, user) {
  if (!userId || !accessToken) return;
  recentSessionsByUser.set(String(userId), { accessToken, refreshToken: refreshToken || null, user: user || { id: userId }, at: Date.now() });
}
...
function refreshSessionOnce(refreshToken) {
  ...
  const promise = anonClient().auth.refreshSession({ refresh_token: refreshToken }).finally(() => {
 ...
 * SECURITY RULE: only Supabase Auth getUser() or a successful refresh may
 * establish identity. An expired/unverified JWT payload is NOT sufficient.
 */
export async function getSession(req, res) {
  ...
  if (token) {
    try {
      ...
      if (!error && data?.user) {
        ...
        return { token, user: data.user, authenticated: true };
      }
    } catch {}
  }

  // A cached session is trusted only because it was previously established by
  // Supabase Auth in this same warm instance. The unverified JWT subject is
  // used only to locate that already-verified cache entry.
  if (jwtSub) {
    const recent = getRecentSession(jwtSub);
    if (recent?.accessToken && recent.user?.id === jwtSub) {
      if (res && recent.refreshToken) setSessionCookie(res, recent.accessToken, recent.refreshToken);
      return { token: recent.accessToken, user: recent.user, authenticated: true, refreshedFromCache: true };
    }
  }
  ...
  if (refreshToken) {
    try {
      ...
      if (!refreshErr && refreshed?.session && refreshed?.user?.id) {
        ...
        rememberSession(u.id, access, refresh, u);
        if (res) setSessionCookie(res, access, refresh);
        return { token: access, user: u, authenticated: true, refreshed: true };
      }
      // A failed refresh may race with another request; only use a cache entry
      // that was previously authenticated by Supabase Auth.
      if (jwtSub) {
        const recent = getRecentSession(jwtSub);
        if (recent?.accessToken && recent.user?.id === jwtSub) {
          if (res && recent.refreshToken) setSessionCookie(res, recent.accessToken, recent.refreshToken);
  ...
  // CRITICAL: do not return jwtSub here. Callers must see an unauthenticated
  // request when the token cannot be verified/refreshed.
