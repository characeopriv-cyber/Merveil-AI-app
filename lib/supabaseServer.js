...
// across weeks the same way Facebook "Stay logged in" does.
...
export async function getSession(req, res) {
  ...
  try {
    ...
    if (refreshErr || !refreshed?.session) {
      // Do NOT clear cookies here — concurrent refresh races often return
      // "Already used" while another request already rotated successfully.
      // Returning null lets this request 401 once; the next focus/keepalive
      // will pick up the new cookies if rotation already happened.
