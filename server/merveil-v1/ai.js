...
export default async function handler(req, res) {
  ...
  catch { return json(res, 400, { error: 'invalid_json', request_id: auth.requestId }); }
  ...
  if ((usage?.message_count || 0) >= limit) {
    await logApiUsage(auth, req, 429);
    return json(res, 429, { error: 'daily_ai_limit_reached', limit, used: usage?.message_count || 0, request_id: auth.requestId });
  }
  try {
    const result = await generate({ messages, maxTokens: b.max_tokens, temperature: b.temperature });
    await supabaseAdmin.rpc('increment_ai_usage', { uid: auth.app.user_id }).catch(() => {});
    await emitWebhookEventSafe('ai.completed', { application_id: auth.app.id, user_id: auth.app.user_id, model: result.model, provider: result.provider, request_id: auth.requestId });
    await logApiUsage(auth, req, 200);
    return json(res, 200, { data: { reply: result.reply, model: 'merveil-intelligence', capability: b.capability || 'general', provider: 'merveil', routing: 'automatic' }, request_id: auth.requestId });
  } catch {
    await logApiUsage(auth, req, 503);
    return json(res, 503, { error: 'merveil_intelligence_unavailable', request_id: auth.requestId });
  }
}
