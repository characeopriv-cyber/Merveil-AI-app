...
export default async function handler(req,res){
  ...
  if(req.method==='POST'){const b=body(req);const url=String(b.url||'').trim();const events=Array.isArray(b.events)?[...new Set(b.events.map(String).filter(Boolean))].slice(0,50):[];if(!/^https:\/\//i.test(url)||url.length>2000)return json(res,400,{error:'invalid_webhook_url'});if(!events.length)return json(res,400,{error:'events_required'});const secret=newSecret('whsec_');let encrypted;try{encrypted=encryptWebhookSecret(secret.value)}catch(e){return json(res,503,{error:'webhook_secret_storage_not_configured'});}const {data,error}=await supabaseAdmin.from('api_webhooks').insert({application_id:appId,url,events,secret_hash:hashKey(secret.value),secret_ciphertext:encrypted}).select('id,url,events,status,created_at,updated_at').single();if(error)return json(res,500,{error:'database_error'});return json(res,201,{data,signing_secret:secret.value,warning:'Store this signing secret securely. It will not be shown again.'});}
