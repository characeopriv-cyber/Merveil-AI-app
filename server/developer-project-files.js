...
const safeDeletePath = p => validPath(p) ? p : null;
...
export default async function developerProjectFiles(req, res) {
  ...
  if (req.method === 'GET') {
    const { data, error } = await db.from('developer_project_files').select('id,path,content,created_at,updated_at').eq('project_id', projectId).eq('owner_user_id', uid).order('path').limit(MAX_FILES);
    if (error) return { status: 500, body: { error: error.message } };
    return { status: 200, body: { ok: true, project, files: data || [], limits: { maxFiles: MAX_FILES, maxFileBytes: MAX_FILE_BYTES, maxTotalBytes: MAX_TOTAL_BYTES } } };
  }
  if (req.method === 'POST') {
    ...
    for (const f of files) {
      ...
      if (bytes > MAX_FILE_BYTES) return { status: 413, body: { error: `File too large: ${path}`, code: 'FILE_TOO_LARGE' } };
      total += bytes; if (total > MAX_TOTAL_BYTES) return { status: 413, body: { error: 'Project is too large', code: 'PROJECT_TOO_LARGE' } };
  ...
  if (req.method === 'DELETE') {
    const path = cleanPath(body.path), safe = safeDeletePath(path);
    if (!safe) return { status: 422, body: { error: `Blocked or unsafe path: ${path || 'empty'}`, code: 'UNSAFE_PATH' } };
    const { error } = await db.from('developer_project_files').delete().eq('project_id', projectId).eq('owner_user_id', uid).eq('path', safe);
    if (error) return { status: 500, body: { error: error.message } };
    return { status: 200, body: { ok: true, deleted: safe } };
  }
