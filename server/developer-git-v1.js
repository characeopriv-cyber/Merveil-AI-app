...
async function requireSuccessfulBuild(client,uid,projectId,buildId){if(!buildId)return{ok:false,error:'A successful Sandbox Build is required before GitHub Push.',code:'BUILD_REQUIRED'};const{data}=await client.from('developer_builds').select('id,status,finished_at').eq('id',String(buildId)).eq('project_id',projectId).eq('owner_user_id',uid).maybeSingle();if(!data||data.status!=='success')return{ok:false,error:'The supplied build did not pass.',code:'BUILD_NOT_VERIFIED'};return{ok:true,build:data}}
export default async function developerGit(req,res){
 ...
 const action=String(body.action||'');try{
  ...
  if(action==='commit'||action==='push'){const fullName=String(body.fullName||project.metadata?.github_repo||''),branch=String(body.branch||project.metadata?.github_branch||'');if(!fullName)return{status:400,body:{error:'Select a GitHub repository first'}};const build=await requireSuccessfulBuild(client,uid,projectId,body.buildId);if(!build.ok)return{status:422,body:build};const{data:files,error}=await client.from('developer_project_files').select('path,content').eq('project_id',projectId).eq('owner_user_id',uid).limit(300);if(error)throw error;const r=await pushRepo(uid,projectId,fullName,branch,files||[],body.message||`Update ${project.name} from Merveil Developer Platform`);await saveBinding(client,projectId,r.repo,r.branch,r.commit);return{status:200,body:{ok:true,action,repo:r.repo,branch:r.branch,commit:r.commit,url:r.url,buildId:build.build.id}}}
