...
export default async function developerGit(req,res){
 ...
 const action=String(body.action||'');try{
  ...
  if(action==='commit'||action==='push'){const fullName=String(body.fullName||project.metadata?.github_repo||''),branch=String(body.branch||project.metadata?.github_branch||'');if(!fullName)return{status:400,body:{error:'Select a GitHub repository first'}};const{data:files,error}=await client.from('developer_project_files').select('path,content').eq('project_id',projectId).eq('owner_user_id',uid).order('path').limit(300);if(error)throw error;const currentSnapshot=snapshotOf(files||[]);const build=await requireSuccessfulBuild(client,uid,projectId,body.buildId,currentSnapshot);if(!build.ok)return{status:422,body:build};const r=await pushRepo(uid,projectId,fullName,branch,files||[],body.message||`Update ${project.name} from Merveil Developer Platform`);await saveBinding(client,projectId,r.repo,r.branch,r.commit);return{status:200,body:{ok:true,action,repo:r.repo,branch:r.branch,commit:r.commit,url:r.url,buildId:build.build.id,snapshot:currentSnapshot}}}
