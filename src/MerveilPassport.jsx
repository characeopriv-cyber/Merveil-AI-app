...
export default function MerveilPassport({ data = null, onNavigate = () => {} }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await fetch("/api/passport-session", { credentials: "include", cache: "no-store", headers: { "Cache-Control": "no-cache" } });
      const body = await r.json().catch(() => ({}));
      if (r.ok && body?.data) { setPassport(body.data); setState("ready"); }
      else setState(body?.error === "Authentication required" ? "auth" : "error");
    } catch { setState("error"); }
    finally {
      loadingRef.current = false;
      if (reloadQueuedRef.current) { reloadQueuedRef.current = false; setTimeout(() => load(), 0); }
    }
  }, []);
  loadRef.current = load;

  useEffect(() => {
    if (data) { setPassport(data); setState("ready"); }
    else load();
  }, [data, load]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const channel = supabase.channel(`merveil-passport-${b.user_id}`);
        const refreshEvent = () => loadRef.current?.();
        channel.on("postgres_changes", { event: "*", schema: "public", table: "profiles", filter: `id=eq.${b.user_id}` }, refreshEvent);
        channel.on("postgres_changes", { event: "*", schema: "public", table: "verifications", filter: `user_id=eq.${b.user_id}` }, refreshEvent);
        channel.on("postgres_changes", { event: "*", schema: "public", table: "passport_signals", filter: `user_id=eq.${b.user_id}` }, refreshEvent);
        channel.subscribe(status => {
          if (status === "SUBSCRIBED") refreshEvent();
          if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") setTimeout(() => alive && refreshEvent(), 2000);
        });
  ...
  if (state === "error" || !passport) return <div className="mpass"><style>{styles}</style><div className="mpass-state"><div><h2>Passport unavailable</h2><p>We couldn't securely load your Passport. Please try again.</p><button className="mpass-btn" onClick={()=>load()}>Retry</button></div></div></div>;
