...
/* The signed transcript is explicitly bound to both call participants/devices.
 * This prevents a valid device key envelope from being replayed into another call.
 */
...
export async function createKeyConfirmation(mediaKey, transcript, challenge) {
  ...
  const material = await crypto.subtle.exportKey('raw', await crypto.subtle.deriveKey({name:'HKDF',hash:'SHA-256',salt:teUtf8('merveil-confirmation-salt'),info:teUtf8(transcript)}, mediaKey, {name:'AES-GCM',length:256}, true, ['encrypt']));
  const hmacKey = await crypto.subtle.importKey('raw', material, {name:'HMAC',hash:'SHA-256'}, false, ['sign']);
  return b64(await crypto.subtle.sign('HMAC', hmacKey, teUtf8(`${transcript}|${challenge}`)));
}
...
export async function verifyKeyConfirmation(mediaKey, transcript, challenge, confirmation) {
  ...
  const expected = await createKeyConfirmation(mediaKey, transcript, challenge);
  const a = ub64(expected); const b = ub64(confirmation);
