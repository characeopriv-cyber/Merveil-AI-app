...
let state = {
  ...
  sidebarOpen: false,
  user: { name: 'You', initials: 'Y', role: 'DESIGNER' },
...
function shell(content) {
  ...
  <div class="shell">
    <aside class="sidebar ${state.sidebarOpen ? 'open' : ''}">
      <div class="sidebar-brand">
        ...
        <div>
          <div class="brand-text">experience</div>
          <div class="brand-sub">UX · journeys · quality</div>
        </div>
      ...
      <div style="margin:0 12px 10px;padding:6px 10px;border-radius:8px;font-size:10px;font-weight:700;letter-spacing:0.06em;text-align:center;background:rgba(99,102,241,0.15);color:#a5b4fc;border:1px solid rgba(129,140,248,0.35)">LIVE · DEVELOPER + STORE</div>
      <nav class="nav">
        ${navItem('home', '⌘', 'Home')}
  ...
  <div class="command-bar" id="ifCmdBar">
    <button type="button" class="cmd-fab-if" id="ifCmdFab">⌘</button>
    <div class="cmd-expand-if" id="ifCmdExpand">
      <input id="cmdInput" type="text" placeholder="Generate UI, journeys, score…" autocomplete="off" />
    </div>
  </div>
...
function viewHome() {
  ...
  <div class="product-card">
    ...
    <p class="product-desc">AI that understands how humans experience technology — connected to code, APIs, agents, Store, and people. Publish from Developer; open here.</p>
...
function loadPublishedProjects() {
  try { return JSON.parse(localStorage.getItem('merveil_interface_catalog_v1') || '[]'); }
  catch { return []; }
}

function viewStore() {
  const published = loadPublishedProjects();
  const arena = [
    { id: 'burj', title: 'Burj Rise', type: 'game', href: '/arena/burj-rise.html', tag: 'Arena' },
    { id: 'connecta', title: 'Connecta', type: 'game', href: '/arena/connecta.html', tag: 'Arena' },
    { id: 'sahra', title: 'Sahra', type: 'game', href: '/arena/sahra.html', tag: 'Arena' },
  ];
  const cards = published.map((p) => `
    <div class="card" data-pub="${esc(p.id)}">
      <div class="tag">${esc((p.project_type || 'project').toUpperCase())} · PUBLISHED</div>
      <h3>${esc(p.title || 'Untitled')}</h3>
      <p>${esc((p.prompt || '').slice(0, 120) || p.company || 'From Developer Platform')}</p>
      <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">
        <button class="btn btn-primary btn-sm" data-action="open-published" data-id="${esc(p.id)}">Open</button>
        <button class="btn btn-light btn-sm" data-action="preview-published" data-id="${esc(p.id)}">Preview</button>
      </div>
    </div>`).join('');
  const arenaCards = arena.map((a) => `
    <div class="card">
      <div class="tag">${a.tag}</div>
      <h3>${esc(a.title)}</h3>
      <p>Official Merveil Arena experience — citizens only in-app.</p>
      <a class="btn btn-light btn-sm" style="margin-top:10px;display:inline-block" href="${a.href}" target="_blank" rel="noopener">Play</a>
    </div>`).join('');
  return `
  <div class="form-panel">
    <h2>Store · published projects</h2>
    <p class="lead">Projects finished in Developer appear here when published. Try · Remix · Open · Arena.</p>
    <h3 style="margin:20px 0 12px;font-size:15px">From Developer Platform</h3>
    <div class="grid-2" style="margin-top:0">
      ${cards || '<div class="card"><div class="tag">EMPTY</div><h3>No published projects yet</h3><p>Build in Developer → Export → Publish to Interface.</p><a class="btn btn-primary btn-sm" style="margin-top:10px;display:inline-block" href="/developer">Open Developer</a></div>'}
    </div>
    <h3 style="margin:28px 0 12px;font-size:15px">Merveil Arena</h3>
    <div class="grid-2" style="margin-top:0">${arenaCards}</div>
...
function bind() {
  const fab = document.getElementById('ifCmdFab');
  const bar = document.getElementById('ifCmdBar');
  if (fab && bar) fab.onclick = (e) => { e.stopPropagation(); bar.classList.toggle('open'); if (bar.classList.contains('open')) document.getElementById('cmdInput')?.focus(); };
...
function handleAction(action, el) {
  switch (action) {
    ...
    case 'preview-published': {
      ...
      const list = loadPublishedProjects();
      const item = list.find((x) => x.id === pid);
      if (!item || !item.html) { toast('Project not found', false); break; }
      const w = window.open('', '_blank');
      if (w) { w.document.write(item.html); w.document.close(); }
      else toast('Popup blocked — allow popups to open project');
      break;
    }
    case 'match-people':
