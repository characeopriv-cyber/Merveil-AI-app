...
export class RolloutService {
  ...
  async create(input: { tenantId: string; actorId: string; fleetId: string; capability: string; parameters?: Record<string, unknown>; safetyClass?: 'read'|'control'|'critical'; stageSize?: number; maxConcurrency?: number; idempotencyKey?: string }) {
    if (!input.actorId || !input.capability?.trim()) throw new BadRequestException('Authenticated actor and capability are required');
    ...
    await this.db.request('machine_connect_fleet_rollouts', { method: 'POST', body: JSON.stringify({ id, organization_id: input.tenantId, fleet_id: input.fleetId, capability: input.capability, parameters: input.parameters ?? {}, safety_class: input.safetyClass ?? 'control', stage_size: stageSize, max_concurrency: maxConcurrency, status: 'pending', requested_by: input.actorId, idempotency_key: key, total: members.length, completed: 0, failed: 0, current_stage: 0 }) });
  ...
  async run(tenantId: string, rolloutId: string) {
    ...
    for (const stage of stages) {
      ...
      for (let i = 0; i < stageItems.length; i += Math.min(rollout.max_concurrency, 25)) {
        const batch = stageItems.slice(i, i + Math.min(rollout.max_concurrency, 25));
  ...
  async pause(tenantId: string, rolloutId: string) { await this.get(tenantId, rolloutId); await this.patch(tenantId, rolloutId, { status: 'paused' }); return this.get(tenantId, rolloutId); }
  async cancel(tenantId: string, rolloutId: string) { await this.get(tenantId, rolloutId); await this.patch(tenantId, rolloutId, { status: 'cancelled', finished_at: new Date().toISOString() }); return this.get(tenantId, rolloutId); }
  ...
  private async executeItem(tenantId: string, rollout: any, item: any, machine: any) {
    ...
    await this.patchItem(tenantId, item.id, { status: 'running', started_at: new Date().toISOString() });
    try {
  ...
  private async finishItem(tenantId: string, itemId: string, status: string, error?: string, commandId?: string) { await this.patchItem(tenantId, itemId, { status, error: error ?? null, command_id: commandId ?? null, finished_at: new Date().toISOString() }); }
  private async patchItem(tenantId: string, id: string, body: Record<string, unknown>) { await this.db.request(`machine_connect_fleet_rollout_items?id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(tenantId)}`, { method: 'PATCH', body: JSON.stringify(body) }); }
  private async patch(tenantId: string, id: string, body: Record<string, unknown>) { await this.db.request(`machine_connect_fleet_rollouts?id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(tenantId)}`, { method: 'PATCH', body: JSON.stringify(body) }); }
  private async recalculate(tenantId: string, id: string) { const items = await this.db.request<any[]>(`machine_connect_fleet_rollout_items?organization_id=eq.${encodeURIComponent(tenantId)}&rollout_id=eq.${encodeURIComponent(id)}`); const completed = items.filter(x => x.status === 'completed').length; const failed = items.filter(x => x.status === 'failed').length; const status = failed === items.length ? 'failed' : failed ? 'partial' : 'completed'; await this.patch(tenantId, id, { completed, failed, status, finished_at: new Date().toISOString() }); return this.get(tenantId, id); }
}
