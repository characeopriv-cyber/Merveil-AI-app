...
describe('MqttAdapter', () => {
  ...
  const credentials = { verify: jest.fn() } as any;
  const telemetry = { append: jest.fn(), heartbeat: jest.fn() } as any;
  const commands = { acknowledgeMachine: jest.fn() } as any;
  ...
  it('stays disabled and disconnected when no broker is configured', () => {
    const adapter = new MqttAdapter(registry, credentials, telemetry, commands);
