...
export class CommandService {
  ...
  async transition(tenantId: string, commandId: string, to: CommandStatus): Promise<MachineCommand> {
    ...
    command.status = to;
    if (this.db.enabled) await this.db.request(`machine_connect_commands?id=eq.${encodeURIComponent(commandId)}&organization_id=eq.${encodeURIComponent(tenantId)}`, { method: 'PATCH', body: JSON.stringify({ status: to }) });
    return command;
  }
}
