...
export class CommandController {
  constructor(
    ...
    private readonly ackSignatures: MachineAckSignatureService,
  ) {}
  ...
  async acknowledgeMachine(@Req() req: RequestWithPrincipal, @Param('machineId') machineId: string, @Param('commandId') commandId: string) {
    ...
    if (principal.actorId !== `machine:${machineId}`) throw new UnauthorizedException('Machine identity does not match ACK target');
    await this.credentials.require(principal.tenantId, machineId, credential);
    await this.ackSignatures.verifyAndConsume({
      tenantId: principal.tenantId,
      machineId,
      commandId,
      headers: {
        signature: this.header(req, 'x-machine-ack-signature'),
        timestamp: this.header(req, 'x-machine-ack-timestamp'),
        nonce: this.header(req, 'x-machine-ack-nonce'),
      },
    });
