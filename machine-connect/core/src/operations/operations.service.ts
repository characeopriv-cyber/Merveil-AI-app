...
export class OperationsService {
  ...
  constructor(private readonly db: SupabaseRest, private readonly machines: MachineService, private readonly closedLoop: ClosedLoopService) {}
  ...
  async evaluate(event: OperationalEvent): Promise<RuleEvaluationResult[]> {
    ...
    for (const rule of candidates) {
      ...
      if (matched) {
        ...
        if (this.db.enabled) await this.db.request('machine_connect_operational_rule_runs', { method: 'POST', body: JSON.stringify({ id: randomUUID(), organization_id: event.tenantId, rule_id: rule.id, event_id: event.id, correlation_id: event.correlationId, status: 'matched', created_at: new Date().toISOString() }) });
        try {
          const action = await this.closedLoop.executeRuleAction(rule, event);
          result.actionId = action.actionId;
          result.commandId = action.commandId;
        } catch (error) {
          result.reason = error instanceof Error ? `action_failed:${error.message}` : 'action_failed';
          if (this.db.enabled) await this.db.request('machine_connect_operational_rule_runs', { method: 'POST', body: JSON.stringify({ id: randomUUID(), organization_id: event.tenantId, rule_id: rule.id, event_id: event.id, correlation_id: event.correlationId, status: 'action_failed', created_at: new Date().toISOString() }) });
        }
      }
      results.push(result);
    }
  ...
  private matches(payload: Record<string, unknown>, condition: RuleCondition): boolean {
    const actual = this.resolve(payload, condition.field); const expected = condition.value;
