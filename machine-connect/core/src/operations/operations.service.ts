...
export class OperationsService {
  ...
  async createRule(input: { tenantId: string; name: string; eventType: string; conditions: RuleCondition[]; enabled?: boolean; cooldownSeconds?: number; action?: OperationalRule['action'] }): Promise<OperationalRule> {
    ...
    const rule: OperationalRule = {
      ...
      eventType: input.eventType.trim().slice(0, 120),
      conditions: input.conditions,
      cooldownSeconds: Math.max(0, Math.min(86400, Math.floor(input.cooldownSeconds ?? 0))),
      action: input.action,
      createdAt: new Date().toISOString(),
    };
    if (this.db.enabled) {
      await this.db.request('machine_connect_operational_rules', {
        method: 'POST',
        body: JSON.stringify({
          id: rule.id,
          organization_id: rule.tenantId,
          name: rule.name,
  ...
  async evaluate(event: OperationalEvent): Promise<RuleEvaluationResult[]> {
    ...
    for (const rule of candidates) {
      if (!rule.enabled) {
        results.push({ ruleId: rule.id, matched: false, reason: 'disabled' });
        continue;
      }
      const last = this.triggered.get(`${event.tenantId}:${rule.id}`) ?? 0;
      if (Date.now() - last < rule.cooldownSeconds * 1000) {
        results.push({ ruleId: rule.id, matched: false, reason: 'cooldown' });
        continue;
      }
      ...
      if (matched) {
        if (!rule.action) {
          results.push({ ...result, reason: 'matched_no_action' });
          continue;
        }
        if (!event.machineId) {
          results.push({ ...result, reason: 'matched_requires_machine' });
          continue;
        }

        // Rules are now orchestration triggers, not a second command-execution path.
        // Durable workflow execution owns retries, leases, idempotency and the existing
        // ClosedLoopService -> CommandService safety boundary.
        try {
          ...
          if (this.db.enabled) {
            await this.db.request('machine_connect_operational_rule_runs', {
              method: 'POST',
              body: JSON.stringify({
                id: randomUUID(),
                organization_id: event.tenantId,
  ...
  private validateAction(action: OperationalRule['action']): void {
    if (!action) return;
    if (action.type === 'command') {
      if (!action.capability || action.capability.length > 160) throw new BadRequestException('invalid command capability');
      if (action.parameters && (typeof action.parameters !== 'object' || Array.isArray(action.parameters) || Object.keys(action.parameters).length > 50)) throw new BadRequestException('invalid command parameters');
      return;
    }
    if (action.type === 'twin_patch') {
      if (!action.state || typeof action.state !== 'object' || Array.isArray(action.state) || Object.keys(action.state).length > 50) throw new BadRequestException('invalid twin patch');
      return;
    }
