...
export class ConnectorSyncService {
  constructor(
    ...
    private readonly ledger: ConnectorSyncLedgerService,
  ) {}

  async sync(organizationId: string, actorId: string, connectorId: string, machineId?: string, idempotencyKey?: string): Promise<any> {
    ...
    if (!IDEMPOTENCY.test(key)) throw new BadRequestException('idempotencyKey must be 8-160 safe characters');

    const claim = await this.ledger.claim({ organizationId, connectorId, machineId: String(targetMachineId), actorId, idempotencyKey: key });
    if (claim.duplicate) {
      return { connectorId, machineId: targetMachineId, telemetryId: claim.telemetryId ?? null, jobId: claim.jobId ?? null, idempotencyKey: key, duplicate: true };
    }
    ...
    try {
      ...
      if (Number.isFinite(contentLength) && contentLength > MAX_RESPONSE_BYTES) throw new BadRequestException('Connector response exceeds the 64 KiB limit');
      ...
      const job = await this.jobs.enqueue(organizationId, actorId, {
        ...
        connectorId,
        payload: { telemetryId: telemetry.id, machineId: targetMachineId, bytes, latencyMs, digest, idempotencyKey: key },
      });
      await this.ledger.complete(organizationId, claim.id, { telemetryId: telemetry.id, jobId: job.id, digest, bytes, latencyMs });

      return { connectorId, machineId: targetMachineId, telemetryId: telemetry.id, jobId: job.id, bytes, latencyMs, digest, idempotencyKey: key, duplicate: false };
    } catch (error) {
      await this.ledger.fail(organizationId, claim.id, error instanceof Error ? error.constructor.name : 'sync_failed');
      throw error;
    }
  }

  private normalizePayload(payload: unknown, bytes: number, latencyMs: number, digest: string): Record<string, unknown> {
    if (payload && typeof payload === 'object' && !Array.isArray(payload)) return { ...(payload as Record<string, unknown>), _connector: { bytes, latencyMs, digest } };
