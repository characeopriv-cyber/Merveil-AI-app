...
function buildSyncEnvelope(row) {
  ...
  return {
    queue_id: String(row.id), table_name: row.table_name, record_id: row.record_id, operation: row.operation,
    data, observed_at: typeof data.timestamp === 'number' ? new Date(data.timestamp * 1000).toISOString() : new Date().toISOString(),
    sequence_no: Number.isSafeInteger(Number(data.sequence)) ? Number(data.sequence) : undefined,
    source: 'offline', schema_version: 1, idempotency_key: `offline-sync:${row.id}`,
  };
}
...
async function handleTransportResult(pool, row, result, maxAttempts = DEFAULT_MAX_ATTEMPTS) {
  ...
  if (result?.conflict) {
    ...
    await pool.query('UPDATE sync_queue SET claimed_at = NULL, claim_id = NULL, last_error = $2 WHERE id = $1 AND claim_id = $3', [row.id, result.reason || 'sync conflict', row.claim_id]); return 'conflict';
  }
  const nextAttempts = attempts + 1;
  if (result?.rejected && nextAttempts >= maxAttempts) {
    await pool.query('UPDATE sync_queue SET attempts = $1, dead_lettered = true, claimed_at = NULL, claim_id = NULL, last_error = $2 WHERE id = $3 AND claim_id = $4', [nextAttempts, result.reason || 'cloud rejected after retry ceiling', row.id, row.claim_id]); return 'dead-lettered';
  }
  ...
  await pool.query('UPDATE sync_checkpoints SET last_failure_at = now(), last_queue_id = $1, last_error = $2 WHERE id = true', [row.id, reason]); return 'retry';
}
