...
self.addEventListener("notificationclick", (event) => {
  ...
  let url = data.url || "/";
  // Deep-link into Connect + open the ringing call when Answer is tapped
  if (action === "answer" && data.callId) {
    url = `/?tab=messages&answerCall=${encodeURIComponent(data.callId)}&callType=${encodeURIComponent(data.callType || "voice")}`;
  } else if (action === "decline" && data.callId) {
    url = `/?tab=messages&declineCall=${encodeURIComponent(data.callId)}`;
  ...
  event.waitUntil(
    self.clients.matchAll({ type: "window", includeUncontrolled: true }).then((clientList) => {
      const payload = { ...data, notificationAction: action, url };
